...
class TestScalingEngine:
    ...
    def test_holding_range_keeps_current_step(self):
        ...
        decision = engine.decide(
            [Sample(72), Sample(71), Sample(70)], Status(False), 9, 0.0
        )
    ...
    def test_critical_util_resets_to_zero_without_experiment(self):
        ...
        decision = engine.decide(
            [Sample(98), Sample(99), Sample(99)], Status(False), 32, 0.0
        )
        assert decision.target_step == 0
        assert decision.filler_mps_cap_pct == 0

    def test_critical_util_resets_to_zero_with_experiment(self):
        engine = ScalingEngine(
            ScalingConfig(
        ...
        decision = engine.decide(
            [Sample(95), Sample(96), Sample(97)], Status(True), 32, 0.0
        )
        assert decision.target_step == 0
        assert decision.filler_mps_cap_pct == 0
